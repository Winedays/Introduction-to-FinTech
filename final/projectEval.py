...
for ic in range(evalDays,0,-1):
    ...
    #print( dateStr )
    #print( minutelyOhlcv.iloc[:,0].str.split(expand=True)[0].values )
    a = np.where(minutelyOhlcv.iloc[:,0].str.split(expand=True)[0].values==dateStr)
    minutelyOhlcvFile = minutelyOhlcv.head((np.where(minutelyOhlcv.iloc[:,0].str.split(expand=True)[0].values==dateStr))[0].max()+1)
    action[evalDays-ic] = 1 #myStrategy(dailyOhlcvFile,minutelyOhlcvFile,openPricev[evalDays-ic])
